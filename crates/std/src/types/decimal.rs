use {
    crate::{forward_ref_partial_eq, Uint128, StdError, StdResult},
...
impl Decimal {
    ...
    pub fn checked_multiply_ratio(
        self,
        _nominator:   impl Into<u128>,
        _denominator: impl Into<u128>,
    ) -> StdResult<Self> {
        todo!("need Decimal256")
    }
}
...
mod tests {
    ...
    fn division() {
        let lhs = Decimal::from_str("74567.67567654").unwrap();
        let rhs = Decimal::from_str("42143.34452434").unwrap();
        assert_eq!(lhs / rhs, Decimal::from_str("1.76938201081").unwrap());
    }
