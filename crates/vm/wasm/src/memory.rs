use {
    ...
    wasmer::{AsStoreMut, AsStoreRef, MemoryError, MemoryType, MemoryView, Pages, Tunables, WasmPtr},
};
...
impl<T: Tunables> MemoryLimit<T> {
    pub fn new(bytes: usize, base: T) -> Self {
        Self { shared_limit : u32::try_from(bytes / wasmer::WASM_MAX_PAGES as usize).unwrap().into(),   base }
    }

    fn ensure_memory(&self, ty: &MemoryType) -> Result<MemoryType, MemoryError> {
        if ty.minimum > self.shared_limit {
            Err(MemoryError::MinimumMemoryTooLarge { min_requested: ty.minimum, max_allowed: self.shared_limit })
        }
        else {
            match ty.maximum {
                Some(max) if max > self.shared_limit => {
                    Err(MemoryError::MaximumMemoryTooLarge { max_requested: max, max_allowed: self.shared_limit })
                }
                None => {
                    Ok(self.limit_max(ty))
                },
                _=> Ok(*ty)
            }
        }
...
impl<T:Tunables> Tunables for MemoryLimit<T> {
    ...
    fn create_host_table(&self, ty: &wasmer::TableType, style: &wasmer::vm::TableStyle) -> Result<wasmer::vm::VMTable, String> {
