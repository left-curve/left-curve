use {
    crate::Region,
    grug_types::{
        encode_sections, from_borsh_slice, to_borsh_vec, Addr, Api, GenericResult, Order, Querier,
...
impl Querier for ExternalQuerier {
    fn query_chain(&self, req: QueryRequest) -> StdResult<QueryResponse> {
        let req_bytes = to_borsh_vec(&req)?;
        ...
        let res: GenericResult<QueryResponse> = from_borsh_slice(res_bytes)?;
