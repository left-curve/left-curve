...
const ProTradeContainer: React.FC<PropsWithChildren<ProTradeProps>> = ({
	...
	return (
		<ProTradeProvider value={{ state, controllers }}>
			{children}
		</ProTradeProvider>
	);
};
...
const ProTradeOrders: React.FC = () => {
	...
	const [activeTab, setActiveTab] = useState<"open order" | "trade history">(
		"open order",
	);
      ...
      cell: ({ row }) => <Cell.Time date={row.original.time} />,
			...
			cell: ({ row }) => (
				<Cell.Number
					...
					value={formatUnits(
						...
						coins[row.original.baseDenom].decimals,
					)}
				/>
			),
		},
					...
					text={formatNumber(
						Decimal(row.original.price)
							.times(
								Decimal(10).pow(
									coins[row.original.baseDenom].decimals -
										coins[row.original.quoteDenom].decimals,
								),
							)
							.toFixed(),
						formatNumberOptions,
		...
		},
		{
			...
			cell: ({ row }) => (
				<Cell.Action
					action={() =>
						showModal(Modals.ProTradeCloseOrder, { orderId: row.original.id })
					}
		...
		<div className="flex-1 p-4 bg-surface-secondary-rice flex flex-col gap-2 shadow-card-shadow pb-20 lg:pb-5 z-10">
			<div className="relative">
				<Tabs
					...
					onTabChange={(tab) =>
						setActiveTab(tab as "open order" | "trade history")
					}
