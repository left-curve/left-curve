...
export const PerpsMarginMode = forwardRef<void, PerpsMarginModeProps>(({ pairId, ticker }) => {
  const { hideModal } = useApp();
  const currentMode = perpsTradeSettingsStore((s) => s.marginModeByPair[pairId]) ?? "cross";
  const setMarginMode = perpsTradeSettingsStore((s) => s.setMarginMode);

  const [selected, setSelected] = useState<MarginMode>(currentMode);

  const onConfirm = () => {
    setMarginMode(pairId, selected);
    hideModal();
  };
  ...
  return (
    <div className="flex flex-col bg-surface-primary-rice md:border border-outline-secondary-gray pt-0 md:pt-6 rounded-xl relative p-4 md:p-6 gap-5 w-full md:max-w-[28rem]">
      <h2 className="text-ink-primary-900 diatype-lg-bold w-full">
        {m["modals.marginMode.title"]({ symbol: ticker })}
      </h2>
      ...
      <div className="flex items-center gap-2">
        <div
          className={twMerge(
            ...
            selected ? "border-primitives-red-light-400" : "border-outline-secondary-gray",
          )}
        >
          {selected ? (
