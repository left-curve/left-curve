...
export const SwippeableAccountCard: React.FC<Props> = ({ cardVisible, setCardVisible }) => {
  ...
  const { setSidebarVisibility, settings } = useApp();
    ...
    <div className="dots flex w-full items-center justify-center gap-3 lg:hidden">
      <div
        ...
        className={twMerge(
          ...
          cardVisible === 0 ? "bg-fg-tertiary-rice" : "bg-foreground-quaternary-rice",
        )}
      />
      <div
        ...
        className={twMerge(
          "w-[10px] h-[10px] rounded-full cursor-pointer transition-colors",
          cardVisible === 1 ? "bg-fg-tertiary-rice" : "bg-foreground-quaternary-rice",
        )}
      />
    </div>
  );
