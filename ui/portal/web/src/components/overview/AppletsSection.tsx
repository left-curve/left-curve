    ...
    <div className="grid grid-cols-[repeat(auto-fill,_minmax(64px,_1fr))] md:place-items-start gap-4 md:gap-8 w-full md:min-h-fit">
      {Object.values(favApplets).map((applet) => (
        <div key={applet.title} className="flex flex-col items-center gap-2">
          <Link
            to={applet.path}
            className="h-16 w-16 md:h-20 md:w-20 shadow-account-card bg-primary-red hover:bg-red-bean-100 transition-all rounded-xl p-[10px]"
          >
      ...
      <div className="flex flex-col items-center gap-2">
        <button
          ...
          className="h-16 w-16 md:h-20 md:w-20 shadow-account-card border-[1.43px] border-rice-100 text-rice-100 hover:bg-bg-secondary-rice transition-all rounded-xl p-[10px] flex items-center justify-center cursor-pointer"
        >
          <IconAddCross />
        </button>
        <p className="min-h-6" />
