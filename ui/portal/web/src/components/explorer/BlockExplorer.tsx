...
const BlockSkeleton: React.FC = () => {
  ...
  return (
    <div className="w-full md:max-w-[76rem] flex flex-col gap-6 p-4 pt-6 mb-16">
      <div className="flex flex-col gap-4 rounded-xl p-4 bg-surface-secondary-rice shadow-account-card text-ink-secondary-700 diatype-m-bold relative overflow-hidden md:min-h-[177.63px] min-h-[208.5px]">
        <h1 className="h4-bold text-ink-primary-900">
      ...
      <div className="flex flex-col gap-6 rounded-md p-4 bg-surface-secondary-rice shadow-account-card relative overflow-hidden text-ink-secondary-700">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-7 gap-3 items-center text-center lg:max-w-[45.5rem]">
          <div>
            ...
            <span className="diatype-m-medium uppercase text-ink-tertiary-500">
              {m["countdown.days"]({ days })}
            </span>
          </div>
          <span className="h1-bold text-ink-primary-900">:</span>
          <div>
            <p className="h1-bold text-ink-primary-900">{hours}</p>
            <span className="diatype-m-medium uppercase text-ink-tertiary-500">
              {m["countdown.hours"]({ hours })}
            </span>
          </div>
          <span className="hidden lg:flex h1-bold text-ink-primary-900">:</span>
          <div>
            <p className="h1-bold text-ink-primary-900">{minutes}</p>
            <span className="diatype-m-medium uppercase text-ink-tertiary-500">
          ...
          <span className="h1-bold text-ink-primary-900">:</span>
          <div>
            <p className="h1-bold text-ink-primary-900">{seconds}</p>
            <span className="diatype-m-medium uppercase text-ink-tertiary-500">
...
const BlockCronsOutcomes: React.FC = () => {
  ...
  const cronOutcomes = Array.isArray(data.searchBlock.cronsOutcomes)
    ? data.searchBlock.cronsOutcomes.map((item: string) => {
        try {
...
const BlockDetails: React.FC = () => {
  ...
  return (
    <div className="flex flex-col rounded-xl p-4 bg-surface-secondary-rice shadow-account-card text-ink-secondary-700 relative overflow-hidden diatype-sm-medium">
