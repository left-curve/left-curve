...
impl<T> EventStatus<T> {
    pub fn as_option_err(&self) -> Option<&str> {
        match self {
            Self::Ok(_) | Self::NotReached => None,
            Self::Failed { error, .. } => Some(error),
        }
    }

    pub fn ok(&self) -> &T {
        if let Self::Ok(event) = self {
            event
        } else {
            panic!("expecting ok, got error");
        }
    }
}
...
#[derive(Serialize, Deserialize, BorshSerialize, BorshDeserialize, Debug, Clone, PartialEq, Eq)]
pub struct TxEvents {
    pub withhold: EventStatus<EvtWithhold>,
    pub authenticate: EventStatus<EvtAuthenticate>,
    pub msgs_and_backrun: EventStatus<MsgsAndBackrunEvents>,
    pub backrun: EventStatus<EvtBackrun>,
    pub finalize: EventStatus<EvtFinalize>,
}

impl TxEvents {
    pub fn new(withhold: EventStatus<EvtWithhold>) -> Self {
        Self {
            withhold,
            authenticate: EventStatus::NotReached,
            msgs_and_backrun: EventStatus::NotReached,
            backrun: EventStatus::NotReached,
            finalize: EventStatus::NotReached,
        }
    }

    pub fn finalize_fails(self, finalize: EventStatus<EvtFinalize>, cause: &str) -> Self {
        fn update<T>(evt: EventStatus<T>, cause: &str) -> EventStatus<T> {
            if let EventStatus::Ok(event) = evt {
                EventStatus::Failed {
                    event,
        ...
        TxEvents {
            ...
            backrun: update(self.backrun, cause),
            finalize,
        }
    }
}
...
impl Default for TxEvents {
    fn default() -> Self {
        Self {
            withhold: EventStatus::NotReached,
            authenticate: EventStatus::NotReached,
            msgs_and_backrun: EventStatus::NotReached,
            backrun: EventStatus::NotReached,
            finalize: EventStatus::NotReached,
