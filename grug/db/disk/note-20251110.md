...
#[derive(Clone)]
struct StateStorage {
    data: Arc<parking_lot::ArcRwLockReadLock<parking_lot::RawRwLock, Data>>,
}
```
...
- While `parking_lot::ArcRwLockReadLock` is not `Clone`, we can wrap it in another `Arc` to make if `Clone`-able.
