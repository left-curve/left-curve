...
This note documents some observations during our testnet-3, and suggetestions for remedying the problems.

An important fact we've neglected until now, is that the DB is used in a multi-thread environment, where there are at least two threads to be considered:

- The ABCI server, that responds to requests from CometBFT, notably requests to flush new batches and committing them;
- The httpd server, that response to query requests from users via a GraphQL API.
...
## Problem 1: committing a batch during a query

In a previous implementation, the `StateStorage` struct is defined as follows:
...
The following problem exists given this implementation: suppose the httpd server is doing a query that involves making multiple reads in the DB; _at the same time_, the ABCI server requests to commit a batch of data into the DB. The following may happen:

1. httpd server reads the first record.[^1] The DB's version (which equals the last finalized block height) is `N` at this time.
2. ABCI server commits a batch of data. The version is incremented to `N+1`.
3. httpd server reads the second record.

As such, part of the query is performed using data from block `N`, while the other part uses data from block `N+1`. This is a classic **race condition**, and the query result can be corrupted.

### Remedy to problem 1

We decided to at [timestamp](https://github.com/facebook/rocksdb/wiki/User-defined-Timestamp) to the state storage column family. That is, when the httpd server makes a query, it must specify exactly at which version (block height) this query is to be performed at. The DB will then only fetch data at that specific block height:
...
The drawback of this is **disk usage** and **performance penalty**. If a record is overwritten in multiple versions, the DB will keep all of them, each suffixed with the version. When reading the record at a specific version, the DB needs to perform a binary search to find the appropriate version. On testnet-3, we observe significant performance degradation over time as the versions accumulate.

To deal with this, we implemented **periodic pruning**, by calling the following function:
...
There are two problems with this. **First, the race condition discussed in the previous section appears again.** The priority data isnt' timestamped, meaning we're only keeping the latest version of it. So it's possible that a part of a query uses priority data from block `N`, while the other part uses block `N+1`.
...
2. The ABCI thread attempts to acquire a write-lock. It has to wait, because a read-lock is not released.
...
1. For WASM VM, the `wasmer` runtime requires all elements on its `FunctionEnv` to be `Send + 'static`.[^2]
