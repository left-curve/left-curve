...
impl QueryCmd {
    pub async fn run(self, rpc_addr: &str) -> anyhow::Result<()> {
        ...
        match self {
            QueryCmd::Info => print_json_pretty(client.query_info().await?),
            QueryCmd::Balance {
                address,
                denom,
            } => print_json_pretty(client.query_balance(address, denom).await?),
            QueryCmd::Balances {
                address,
                start_after,
                limit,
            } => print_json_pretty(client.query_balances(address, start_after, limit).await?),
            QueryCmd::Supply {
                denom,
            } => print_json_pretty(client.query_supply(denom).await?),
            QueryCmd::Supplies {
                start_after,
                limit,
            } => print_json_pretty(client.query_supplies(start_after, limit).await?),
            QueryCmd::Code {
                hash,
            ...
            QueryCmd::Codes {
                start_after,
                limit,
            } => print_json_pretty(client.query_codes(start_after, limit).await?),
            QueryCmd::Account {
                address,
            } => print_json_pretty(client.query_account(address).await?),
            QueryCmd::Accounts {
                start_after,
                limit,
            } => print_json_pretty(client.query_accounts(start_after, limit).await?),
            QueryCmd::WasmRaw {
                contract,
                key_hex,
...
async fn query_code(client: &Client, hash: Hash) -> anyhow::Result<()> {
    ...
    // make the query
    let wasm_byte_code = client.query_code(hash).await?;
...
async fn query_wasm_raw(
    ...
    print_json_pretty(client.query_wasm_raw(contract, key).await?)
}
