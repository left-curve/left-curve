use {
    ...
    graphql_ws_client::WsClient,
    indexer_graphql_types::{SubscribeBlock, block, subscribe_block},
    indexer_historical_types::{AnyResult, post_graphql},
    reqwest::IntoUrl,
    std::time::Duration,
    url::Url,
};
...
impl HttpdClient {
    ...
    /// `indexer-graphql-types`: it carries a heavier payload than strictly
    ...
    /// `SubscribeBlock` query avoids touching `indexer-graphql-types`; if the
    /// payload size becomes a concern, we can add a dedicated lightweight
    /// subscription later.
    pub(crate) async fn subscribe_blocks(
        &self,
    ) -> AnyResult<BoxStream<'static, AnyResult<u64>>> {
        let stream = self
            .ws
