...
2. **Cache files on disk** — written by the node's `indexer-cache` crate at:
   ```
...
## Coupling with `indexer-cache`

The source depends on the concrete shape of `CacheFile` and
`BlockAndBlockOutcomeWithHttpDetails` in the `indexer-cache` crate. Any
...
invariant: a refactor in `indexer-cache` either updates the source or
fails to build.
