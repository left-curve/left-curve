...
describe("decodeEndian32", () => {
  describe("big-endian", () => {
    it("should decode 32-bit big-endian bytes into a number", () => {
      ...
      const decoded = decodeEndian32(bytes, true);
    ...
    it("should throw an error if byte array is not 4 bytes in length", () => {
      expect(() => decodeEndian32(new Uint8Array([0x12, 0x34, 0x56]), true)).toThrow(
        "expecting exactly 4 bytes, got 3",
      );
      expect(() => decodeEndian32(new Uint8Array([0x12, 0x34, 0x56, 0x78, 0x90]), true)).toThrow(
