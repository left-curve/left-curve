...
:::warning
`getBalance` returns a `number` and will overflow for amounts above 2^53. For accurate accounting use [`getBalances`](../actions/app/getBalances) (returns `Coins`, i.e. `Record<Denom, string>`) and parse with `Decimal` or `BigInt`.
:::
