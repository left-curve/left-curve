...
:::warning
the Dango account address is decoupled from the signing key. The same secp256k1 key can control multiple Dango accounts. Always set `DANGO_ACCOUNT_ADDRESS` explicitly — never derive it from the wallet's EVM address.
:::
