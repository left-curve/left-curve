...
> **Warning:** the Uint64 derived from the cloid is the only thing the chain sees. If two different 16-byte cloids hash to the same Uint64 (probability ~2**-32 over ~4 billion cloids), the cancel applies to whichever order claimed that Uint64 first.
