...
:::note
as of this writing, the SDK returns the BroadcastTxOutcome envelope unchanged from `Exchange._send_action`. `TxFailed` is defined for callers and downstream tooling to raise; the SDK does not raise it automatically. Inspect the returned dict's `check_tx.code` and `result.err` fields to detect failures.
:::
