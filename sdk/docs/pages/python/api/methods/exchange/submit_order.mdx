...
**`size`** — `float | int | str | Decimal`. Signed quantity. Positive = buy, negative = sell. Zero is rejected (`ValueError`). Routed through [`dango_decimal`](../../helpers/dango_decimal) — precision capped at 6 places.
...
- Market and IOC limit orders revert with "no liquidity at acceptable price" if nothing fills; GTC remainders rest on the book and reserve `|opening_size| × limit_price × imr` until cancelled or filled.
