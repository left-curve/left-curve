...
> **Warning:** A wrong password fails decryption with an AES-GCM authentication-tag error — the returned `anyhow::Error` does not distinguish "wrong password" from "file corrupted". Treat all decryption errors the same.
