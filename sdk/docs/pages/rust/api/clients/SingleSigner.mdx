...
`Defined<T>` and `Undefined<T>` are grug typestate helpers. The compiler enforces both fields are `Defined` before `sign_transaction` is callable.
...
| Trait | Where | Methods |
|-------|-------|---------|
| `Addressable` (`grug`) | all states | `address()` |
| `Signer` (`grug`) | `Defined<UserIndex> + Defined<Nonce>` | [`unsigned_transaction`](../methods/single-signer/unsigned_transaction), [`sign_transaction`](../methods/single-signer/sign_transaction) |
