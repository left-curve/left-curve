...
**`hash`** — `grug::Hash256`. The transaction hash returned by [`broadcast_tx`](./broadcast_tx).

## Returns

**`grug::SearchTxOutcome`** — full outcome with `hash`, `height`, transaction body, gas usage, success/failure result, and emitted events.
