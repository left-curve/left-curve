...
From `impl QueryClient for HttpClient`. `UnsignedTx` and `TxOutcome` come from `grug`.
...
**`tx`** — `grug::UnsignedTx`. The transaction body (sender, messages, metadata) without a signature.

## Returns

**`grug::TxOutcome`** — the simulated outcome, including `gas_used`, `gas_limit`, the success/failure result, and emitted events.
