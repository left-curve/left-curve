...
async fn query_store(
    ...
    height: Option<u64>,
    prove: bool,
) -> Result<(Option<Binary>, Option<dango_primitives::Proof>), anyhow::Error>;
```
...
use {
    ...
    dango_primitives::{Binary, QueryClient},
};
...
**`(Option<Binary>, Option<dango_primitives::Proof>)`** — the value at the given key (or `None` if absent) and an optional proof. The proof is returned only when `prove = true` and the node has the witness available.
