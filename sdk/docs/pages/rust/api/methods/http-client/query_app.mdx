...
Run an app-level query against the node.
...
use {
    ...
    dango_primitives::{Query, QueryClient, QueryConfigRequest},
};

#[tokio::main]
async fn main() -> Result<()> {
    let client   = HttpClient::new("https://api-mainnet.dango.zone")?;
    let response = client.query_app(Query::Config(QueryConfigRequest {})).await?;
    println!("{response:?}");
    Ok(())
}
```
