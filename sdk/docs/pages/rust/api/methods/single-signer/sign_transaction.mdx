...
**`grug::Tx`** — `sender`, `msgs`, `gas_limit`, JSON-encoded metadata, and a JSON-encoded `Credential::Standard { key_hash, signature }`.
...
- Errors are `grug::StdError` from JSON encoding or the `Secret::sign_transaction` call.
