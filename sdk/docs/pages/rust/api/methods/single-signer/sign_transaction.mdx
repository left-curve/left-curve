...
impl<S: Secret> Signer for SingleSigner<S> {
    fn sign_transaction(
        ...
        chain_id: &str,
        gas_limit: u64,
    ) -> dango_primitives::StdResult<dango_primitives::Tx>;
}
```
...
use {
    ...
    dango_primitives::{BroadcastClient, Coins, Message, NonEmpty, Signer},
};
...
async fn submit(
    ...
    let messages = NonEmpty::new(vec![
        Message::transfer(dango_primitives::Addr::mock(1), Coins::one("bridge/usdc", 100_u128)?)?,
    ])?;
