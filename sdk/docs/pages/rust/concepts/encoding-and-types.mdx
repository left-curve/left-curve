...
`dango-sdk` is thin: most user-facing types are re-exported by the SDK, `dango_types`, and `dango_indexer_graphql_types`.
...
| `PageInfo`, `<query>::Variables`, `<query>::ResponseData`, generated `*Nodes`/`*Edges` | `dango_indexer_graphql_types` | GraphQL codegen surface, re-exported at the `dango_sdk` crate root. |
| `SubscribeBlock`, …, `SubscribeQueryStatus` (×14) | `dango_indexer_graphql_types` | One marker struct per subscription. Each has a snake-case module of `Variables`/`ResponseData`. |
...
```rust
use {dango_primitives::Addr, std::str::FromStr};
...
```rust
use {dango_primitives::Hash256, std::str::FromStr};
