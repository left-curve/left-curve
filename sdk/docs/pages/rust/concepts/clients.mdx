...
It implements four grug traits, which is where the queries live:
...
### Methods from `grug::QueryClientExt`

Because `HttpClient` implements `QueryClient`, the grug blanket trait `QueryClientExt` lights up a suite of convenience methods. These are owned by `grug`, not `dango-sdk`:
