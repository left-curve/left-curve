...
class Trade(TypedDict):  # noqa: N815
    ...
    # (indexer/graphql-types/src/schemas/schema.graphql) and API doc §8.2. Keys stay
...
class PerpsEvent2(TypedDict):
    """One perps-contract event from the /perps/events/stream feed; camelCase (wire shape)."""

    # Mirrors the perps event the indexer emits on `/perps/events/stream`.
...
class PerpsEvent2Batch(TypedDict):  # noqa: N815
    """One block's matching perps events from the /perps/events/stream feed."""

    # Each Server-Sent event on `subscribe_perps_events2` is one of these: the
