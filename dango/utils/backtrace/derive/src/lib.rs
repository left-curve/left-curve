...
pub fn backtrace(_attr: TokenStream, input: TokenStream) -> TokenStream {
            ...
            match &mut variant.fields {
                Fields::Named(fields) => {
                    ...
                    builder_impl.push(quote! {
                        #pub_ident #fn_name(#(#inputs)*) -> Self {
                            Self::#variant_ident {
                                #(#values)*
                                backtrace: ::dango_backtrace::BT::default(),
                            }
                        }
                    });
                },
                Fields::Unnamed(unamed) => {
                    ...
                    field.ty = parse_quote! {
                        ::dango_backtrace::BacktracedError<#original_ty>
                    };
                    ...
                    if is_fresh {
                        impl_from.push(quote! {
                            impl From<#original_ty> for #input_ident {
                                fn from(t: #original_ty) -> Self {
                                    Self::#variant_ident(::dango_backtrace::BacktracedError::new(t))
                                }
                            }
                        });
                    } else {
                        impl_from.push(quote! {
                            impl From<#original_ty> for #input_ident {
                                fn from(t: #original_ty) -> Self {
                                    let bt = ::dango_backtrace::Backtraceable::backtrace(&t);
                                    Self::#variant_ident(::dango_backtrace::BacktracedError::new_with_bt(t, bt))
                                }
                            }
                        });
                    }
                    ...
                    builder_impl.push(quote! {
                        #pub_ident #fn_name(self, value: #original_ty) -> Self {
                            Self::#variant_ident(::dango_backtrace::BacktracedError::new(value))
                        }
                    });
                },
                Fields::Unit => {
                    ...
                    builder_impl.push(quote! {
                        ...
                        Self::#variant_ident(::dango_backtrace::BT::default())
                       }
                    });
                },
            }
    ...
    quote! {
        ...
        impl ::dango_backtrace::Backtraceable for #input_ident {
            fn into_generic_backtraced_error(self) -> ::dango_backtrace::BacktracedError<String> {
                ::dango_backtrace::BacktracedError::new_with_bt(self.to_string(), self.backtrace())
            }

            fn backtrace(&self) -> ::dango_backtrace::BT {
                match self {
                    #(#match_statement)*
                }
            }
