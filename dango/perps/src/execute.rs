...
fn modify_position(ctx: &mut MutableCtx, denom: Denom, amount: Int128) -> anyhow::Result<()> {
    ...
    if new_size.is_non_zero() {
            ...
            price.value_of_unit_amount::<18>(new_size.unsigned_abs())?
                >= params.min_position_size.checked_into_dec::<18>()?,
    ...
    let fill_price = PrecisionedPrice::new(fill_price, ctx.block.timestamp, price.precision());
