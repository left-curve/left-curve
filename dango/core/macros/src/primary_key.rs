...
pub fn process(input: TokenStream) -> TokenStream {
    ...
    let raw_keys_arms = variant_idents.iter().zip(&indices).map(|(ident, idx)| {
        quote! {
            #name::#ident => ::std::vec![::dango_storage::RawKey::Fixed8([#idx])],
        }
    });
    ...
    quote! {
        impl ::dango_storage::PrimaryKey for #name {
            type Output = Self;
            type Prefix = ();
            type Suffix = ();
            ...
            fn raw_keys(&self) -> ::std::vec::Vec<::dango_storage::RawKey<'_>> {
                match self {
                    #(#raw_keys_arms)*
                }
            }

            fn from_slice(bytes: &[u8]) -> ::dango_storage::__private::StdResult<Self::Output> {
                match bytes {
                    #(#from_slice_arms)*
                    _ => ::std::result::Result::Err(::dango_storage::__private::StdError::deserialize::<
                        Self::Output,
                        _,
                        ::dango_storage::__private::Binary,
                    >(
                        "key",
                        #error_msg,
                        bytes.into(),
