...
pub fn process(attrs: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        impl ::dango_storage::IndexList<#pk, #ty> for #struct_ty<'_> {
            fn get_indexes(&'_ self) -> Box<dyn Iterator<Item = &'_ dyn ::dango_storage::Index<#pk, #ty>> + '_> {
                let v: Vec<&dyn ::dango_storage::Index<#pk, #ty>> = vec![#(#names),*];
                Box::new(v.into_iter())
            }
        }
    }
