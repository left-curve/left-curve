...
pub fn process(attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    quote::quote! {
        ...
        impl ::dango_primitives::EventName for #input_name {
            const EVENT_NAME: &'static str = #name;
        }

        impl TryFrom<#input_name> for ::dango_primitives::ContractEvent {
            type Error = ::dango_primitives::StdError;

            fn try_from(event: #input_name) -> ::dango_primitives::StdResult<Self> {
                Self::new(&event)
            }
        }
    }
