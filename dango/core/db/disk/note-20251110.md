...
This note documents some observations from our testnet-3, and suggestions for remedying the problems.
...
This however, doesn't work, due to limitations of the `dango_app::Vm` trait:

1. For the WASM VM, the `wasmer` runtime requires all elements in a functional environment to be `Send + 'static`.[^2]
2. The Dango app requires the storage instance to be `Clone`.
...
To solve these problems, we use the [`parking_lot`](https://crates.io/crates/parking_lot) library:

```rust
struct StateStorage {
    guard: parking_lot::ArcRwLockReadLock<parking_lot::RawRwLock, Data>,
}
```

- `parking_lot` provides an `ArcRwLockReadLock` (with `arc_clone` feature enabled) which allow borrowing with `'static` lifetime.
- `parking_lot`'s `RwLockReadGuard` is `Send` (with the `send_guard` feature enabled), thanks it being pure Rust, thus not subject to the limitation of the OS.
- While `parking_lot::ArcRwLockReadLock` is not `Clone`, we can implement a custom cloning method:

  ```rust
  impl Clone for StateStorage {
      fn clone(&self) -> Self {
          Self {
              guard: ArcRwLockReadLock::rwlock(&self.guard).read_arc(),
          }
      }
  }
  ```

- `parking_lot` is also faster than `std::sync::RwLock`, at the expense of bigger binary sizes. This isn't a problem in practice, because `tokio` and other popular crates already include a copy of `parking_lot` in their dependency trees anyways.
