    ...
    anyhow::Error: From<Q::Error>,
{
    fn query_price(
        ...
        match price_source {
            ...
            PriceSource::LendingLP {} => {
                // Get the price of the underlying asset
                let Some(underlying_denom) = denom.strip(&[&NAMESPACE, &SUBNAMESPACE]) else {
                    bail!("not a lending pool token: {}", denom)
                };
                ...
                let market = self.query_wasm_path(
                ...
                let price = PrecisionedPrice::new(
