...
pub const VOLUMES: Map<(&Addr, Timestamp), Udec256> = Map::new("volume");

/// Stores the total trading volume in USD for each username and timestamp.
pub const VOLUMES_BY_USER: Map<(&Username, Timestamp), Udec256> = Map::new("volume_by_user");
...
/// ```
pub type LimitOrderKey = ((Denom, Denom), Direction, Udec256, OrderId);
