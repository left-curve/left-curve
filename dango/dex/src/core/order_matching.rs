use {
    ...
    grug::{Number, NumberConst, StdResult, Udec256},
};
...
where
    B: Iterator<Item = StdResult<(Udec256, Order)>>,
    A: Iterator<Item = StdResult<(Udec256, Order)>>,
{
    let mut bid = bid_iter.next().transpose()?;
    let mut bids = Vec::new();
    let mut bid_is_new = true;
    let mut bid_volume = Udec256::ZERO;
    let mut ask = ask_iter.next().transpose()?;
    let mut asks = Vec::new();
    let mut ask_is_new = true;
    let mut ask_volume = Udec256::ZERO;
    let mut range = None;
