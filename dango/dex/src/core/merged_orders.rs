use {
    ...
    grug::{Order as IterationOrder, StdResult, Udec256},
    std::{cmp::Ordering, iter::Peekable},
};

pub struct MergedOrders<A, B>
where
    A: Iterator<Item = StdResult<(Udec256, LimitOrder)>>,
    B: Iterator<Item = (Udec256, PassiveOrder)>,
{
    ...
    B: Iterator<Item = (Udec256, PassiveOrder)>,
{
    ...
    fn next_real(&mut self) -> Option<StdResult<(Udec256, Order)>> {
    ...
    fn next_passive(&mut self) -> Option<StdResult<(Udec256, Order)>> {
...
where
    ...
    B: Iterator<Item = (Udec256, PassiveOrder)>,
{
    type Item = StdResult<(Udec256, Order)>;
