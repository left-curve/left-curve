...
pub fn process(attrs: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        impl ::grug_storage::IndexList<#pk, #ty> for #struct_ty<'_> {
            fn get_indexes(&'_ self) -> Box<dyn Iterator<Item = &'_ dyn ::grug_storage::Index<#pk, #ty>> + '_> {
                let v: Vec<&dyn ::grug_storage::Index<#pk, #ty>> = vec![#(#names),*];
                Box::new(v.into_iter())
            }
        }
    }
