...
//! real-time data, served to clients over the REST/SSE endpoints
//! `/perps/events/stream` and `/block/full/stream`:
//!
...
//!   `app_cfg` there).
//! - [`Context`] — the reader handle the httpd holds; the
//!   `/perps/events/stream` and `/block/full/stream` SSE handlers live in the
//!   httpd crate and drive [`RecentStream::subscribe`].
//!
//! # Future direction
//!
