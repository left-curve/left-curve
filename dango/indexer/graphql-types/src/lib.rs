...
/// Extension trait for [`Variables`] that adds a `.subscribe(&client)` method,
/// returning a [`dango_graphql_ws_client::SubscriptionStream`].
///
/// Implemented automatically for every type generated by the
/// `generate_subscription_types!` macro below.
pub trait SubscriptionVariables: Variables {
    fn subscribe(
        self,
        client: &dango_graphql_ws_client::WsClient,
    ) -> impl std::future::Future<
        Output = anyhow::Result<
            dango_graphql_ws_client::SubscriptionStream<
                <Self::Query as graphql_client::GraphQLQuery>::ResponseData,
            >,
        >,
    > + Send
    where
        Self: Sized + Unpin + Send + Sync + 'static,
        Self::Query: Unpin + Send + Sync + 'static,
        <Self::Query as graphql_client::GraphQLQuery>::ResponseData:
            serde::de::DeserializeOwned + Unpin + Send + Sync + 'static,
    {
    ...
    ($({name: $name:ident, path: $path:literal $(,)?}), * $(,)? ) => {
        $(
            ...
            paste::paste! {
                ...
                impl SubscriptionVariables for [<$name:snake>]::Variables {}
            }
        )*
    };
}
