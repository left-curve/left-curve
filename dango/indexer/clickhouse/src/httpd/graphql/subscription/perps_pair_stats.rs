...
#[derive(Default)]
pub struct PerpsPairStatsSubscription;

#[Subscription]
impl PerpsPairStatsSubscription {
    /// Stream real-time 24h statistics for all perps trading pairs.
    ///
    /// Returns the current cached snapshot immediately, then emits an updated
    /// snapshot each time a new block is indexed.
    async fn all_perps_pair_stats<'a>(
        &self,
