...
How the `archive` service exposes its internal state — **metrics**
(Prometheus pull) and **structured logs** (per-component `tracing` spans) — so an
operator can answer, from Grafana and Loki alone:

- _Is each projection keeping up, and at what height?_
- _Where is the block source spending time — backfill, live tail, store writes?_
- _Which queue is the bottleneck?_
- _Is RocksDB healthy (compaction backlog, write stalls, cache pressure)?_
- _Which read queries are slow?_

This resolves the **Observability** open question in [`../DESIGN.md`](../DESIGN.md).

The intent (the _roles_ below) is fixed; the concrete metric names, label sets,
...
A log line tagged `projection{id=activity}` or `bsource.healer` tells you _who_
...
| -------------------------------------------- | ------- | ----------------------------------------------------- | ------------------------------------------------------------------------ |
| `archive_block_source_frontier`              | gauge   | —                                                     | highest contiguous height reachable via `get` — the source's sync anchor |
| `archive_block_source_live_height`           | gauge   | —                                                     | highest height seen on the live subscription (the observed chain tip)    |
| `archive_block_source_live_blocks_total`     | counter | —                                                     | blocks received from the live subscription                               |
| `archive_block_source_broadcast_sent_total`  | counter | —                                                     | frontier-advance broadcasts emitted to projections                       |
| `archive_block_source_reconnects_total`      | counter | `reason` ∈ subscribe_failed/stream_error/stream_ended | live re-subscribes                                                       |
| `archive_block_source_discontinuities_total` | counter | —                                                     | live-tail holes detected (`height > prev+1`)                             |
...
| Metric                                         | Type      | Labels | Meaning                                             |
| ---------------------------------------------- | --------- | ------ | --------------------------------------------------- |
| `archive_block_source_healing`                 | gauge 0/1 | —      | a gap backfill is in progress                       |
| `archive_block_source_gap_low`                 | gauge     | —      | lowest missing height (gap `from`); 0 when gap-free |
| `archive_block_source_gap_high`                | gauge     | —      | top of the lowest gap (`to`); 0 when gap-free       |
| `archive_block_source_backfill_failures_total` | counter   | —      | gap contiguity-check failures (misbehaving fetcher) |

### Fetcher — backfill throughput (sentinel)

| Metric                                           | Type      | Labels                             | Meaning                                                         |
| ------------------------------------------------ | --------- | ---------------------------------- | --------------------------------------------------------------- |
| `archive_block_fetcher_blocks_total`             | counter   | —                                  | blocks delivered by the fetcher — `rate()` ⇒ **recovery speed** |
| `archive_block_fetcher_requests_total`           | counter   | `outcome` ∈ ok/error/timeout/empty | `/block/full/range` calls                                       |
| `archive_block_fetcher_request_duration_seconds` | histogram | —                                  | per range HTTP call                                             |
...
| Metric                      | Type  | Labels                                    | Meaning                           |
| --------------------------- | ----- | ----------------------------------------- | --------------------------------- |
| `archive_channel_depth`     | gauge | `channel` ∈ broadcast/coordinator/fetcher | queued items                      |
...
| Metric                                       | Type      | Meaning                                                                        |
| -------------------------------------------- | --------- | ------------------------------------------------------------------------------ |
| `archive_block_store_put_duration_seconds`   | histogram | persist + checkpoint one block (encode + `WriteBatch` write) — **commit time** |
...
| Metric                                        | Type      | Labels       | Meaning                                                                  |
| --------------------------------------------- | --------- | ------------ | ------------------------------------------------------------------------ |
| `archive_projection_height`                   | gauge     | `projection` | last committed height (from the committer — one place sees every cursor) |
...
| `archive_projection_lagged_total`             | counter   | `projection` | broadcast-overflow → Phase-1 fallbacks (broadcast under-sized)           |
...
| `archive_query_total`                   | counter   | `query`, `outcome` ∈ ok/error | per-feed executions                              |
| `archive_http_request_duration_seconds` | histogram | —                             | end-to-end HTTP request handling                 |
| `archive_http_requests_total`           | counter   | —                             | HTTP requests served                             |
| `archive_http_in_flight`                | gauge     | —                             | concurrent in-flight HTTP requests               |
...
| Metric                                | Type    | Labels              | Meaning                                                 |
| ------------------------------------- | ------- | ------------------- | ------------------------------------------------------- |
| `archive_block_loader_failures_total` | counter | —                   | block hydrations that errored (served as "unavailable") |
