use {
    ...
    grug::{
        btree_map, Addr, Addressable, Binary, Coins, ContractWrapper, Denom, HashExt, JsonSerExt,
        Message, MsgConfigure, NextNumber, NonEmpty, Number, NumberConst, PrevNumber, ResultExt,
        Udec128, Uint128,
    },
...
proptest! {
    ...
    fn test_liquidation_scenario(scenario in liquidation_scenario()) {
        ...
        for debt in &scenario.debts {
            ...
            // Mint debt denom to the relayer account
            let relayer = accounts.relayer.address();
            mint_coins(
                &mut suite,
                &mut accounts,
                &contracts,
            ...
            // Provide to lending market from the relayer account
            suite
                .execute(
                    &mut accounts.relayer,
                    contracts.lending,
        ...
        let liquidation_event = res.events.iter().find(|e| e.r#type == "execute" && e.attributes.iter().any(|a| a.key == "liquidation_bonus")).unwrap();
        let liquidation_bonus: Udec128 = liquidation_event.attributes.iter().find(|a| a.key == "liquidation_bonus").unwrap().value.parse().unwrap();
        prop_assert!(
