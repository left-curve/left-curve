...
pub fn do_upgrade<VM>(storage: Box<dyn Storage>, _vm: VM, _block: BlockInfo) -> AppResult<()> {
    ...
    {
        let volume = volume.checked_mul(SCALING_FACTOR).map_err(StdError::from)?;
        dango_dex::VOLUMES.save(&mut dex_storage, (&address, timestamp), &volume)?;
    }
    ...
    {
        let volume = volume.checked_mul(SCALING_FACTOR).map_err(StdError::from)?;
        dango_dex::VOLUMES_BY_USER.save(&mut dex_storage, (&username, timestamp), &volume)?;
    }
