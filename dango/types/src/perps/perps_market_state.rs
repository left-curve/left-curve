...
use grug::{
    Bounded, Dec128, Denom, Exponentiate, Inner, Int128, IsZero, MathError, MultiplyFraction,
    Number, NumberConst, Sign, Signed, Timestamp, Udec128, Uint128, Unsigned,
...
impl PerpsMarketState {
    ...
    pub fn calculate_order_fee(
        ...
        let fee_usd = if same_side(skew, new_skew) {
            ...
            fee_rate
                .into_inner()
                .checked_mul(notional_diff.unsigned_abs().checked_into_dec::<18>()?)?
        } else {
    ...
    pub fn unrealized_price_pnl(
        ...
        let skew = self.skew()?.checked_into_dec::<18>()?;
        let skew_scale = skew_scale.checked_into_dec::<18>()?.checked_into_signed()?;
        let cost_basis_sum = self.accumulators.cost_basis_sum;
        let quadratic_fee_basis = self
            .accumulators
            .quadratic_fee_basis
            .checked_into_dec::<18>()?;
