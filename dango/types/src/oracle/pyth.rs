...
impl PythVaa {
    pub fn verify(&self) -> StdResult<Vec<PriceFeed>> {
        todo!()
    }

    pub fn new<T>(bytes: T) -> StdResult<Self>
    where
        T: Into<Vec<u8>>,
    {
        ...
        let (vaa, feeds) = if bytes[0..4] == *PYTHNET_ACCUMULATOR_UPDATE_MAGIC {
            let res = AccumulatorUpdateData::try_from_slice(&bytes).unwrap();
            ...
            let feeds: Vec<_> = updates
                ...
                .map(|update| {
                    ...
                    let msg = from_slice::<BigEndian, Message>(&message_vec).unwrap();
            ...
            let vaa = VAA::new(bytes)?;
            let batch_attestation = BatchPriceAttestation::deserialize(&vaa.payload[..]).unwrap();
...
mod tests {
    ...
    fn des_pyth_vaa() {
        ...
        let vaa = serde_json::from_str::<PythVaa>(str).unwrap();
