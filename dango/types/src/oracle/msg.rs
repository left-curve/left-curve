use {
    crate::oracle::{Price, PriceSourceWithWeight},
...
pub struct InstantiateMsg {
    pub price_sources: BTreeMap<Denom, Vec<PriceSourceWithWeight>>,
...
pub enum ExecuteMsg {
    ...
    RegisterPriceSources(BTreeMap<Denom, Vec<PriceSourceWithWeight>>),
...
pub enum QueryMsg {
    ...
    /// Query the price sources of the given denom.
    #[returns(Vec<PriceSourceWithWeight>)]
    PriceSource { denom: Denom },
    /// Enumerate the price sources of all supported denoms.
    #[returns(BTreeMap<Denom, Vec<PriceSourceWithWeight>>)]
    PriceSources {
        start_after: Option<Denom>,
        limit: Option<u32>,
    },
