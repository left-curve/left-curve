...
const LoginFlow: React.FC<Props> = ({ changeSelection, onFinish }) => {
  return (
    <WizardContainer onReset={() => changeSelection("register")} onFinish={onFinish}>
      <Step1 />
      <Step2 />
    </WizardContainer>
  );
...
const Step2: React.FC = () => {
  ...
  const loginWithPasskey = async () => {
    const { signature } = await requestWebAuthnSignature({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col w-full gap-3">
        ...
        <Button color="primary" variant="flat" onClick={() => loginWithExternalWallet("backpack")}>
          Backpack
        </Button>
        <Button color="primary" variant="flat" onClick={() => loginWithExternalWallet("metamask")}>
          Metamask
        </Button>
        <Button color="primary" variant="flat" onClick={() => loginWithExternalWallet("phantom")}>
          Phantom
        </Button>
      </div>
      <div className=" w-full">
        <div className="flex gap-1 start text-sm w-full">
          <p>Not {data.userId}?</p>
          <Button className="text-sm " variant="link" size="none" onClick={previousStep}>
            Switch ID
          </Button>
        </div>
      </div>
